...
coord_astrometric_names = ("phi2", "parallax", "pmphi1")
...
bkg_plx_model = sml.builtin.Exponential(
    ...
    coord_err_names=("parallax_err",),
    coord_bounds={"parallax": coord_bounds["parallax"]},
    params=ModelParameters(
        {
            "parallax": {
                "slope": ModelParameter(bounds=SigmoidBounds(-3.0, -0.5), scaler=None)
            }
        }
    ),
)
...
pm_names = ("pmphi1",)
bkg_pm_model = sml.builtin.Normal(
    net=sml.nn.sequential(
        data=1, hidden_features=64, layers=4, features=2, dropout=0.15
    ),
    data_scaler=scaler,
    indep_coord_names=("phi1",),
    coord_names=pm_names,
    coord_err_names=tuple(f"{k}_err" for k in pm_names),
    coord_bounds={k: coord_bounds[k] for k in pm_names},
    params=ModelParameters(
        {
            "pmphi1": {
                "mu": ModelParameter(
                    bounds=SigmoidBounds(-10.0, 10),
                    scaler=StandardLocation.from_data_scaler(scaler, "pmphi1"),
                ),
                "ln-sigma": ModelParameter(
                    bounds=SigmoidBounds(1.1, 2.1),
...
stream_cp = QTable(
    {
        "phi1": [130, 150, 159, 167] * u.deg,
        "phi2": [36.25, 36, 34.75, 33] * u.deg,
        "w_phi2": [0.5, 0.5, 0.5, 0.5] * u.deg,
        "parallax": [0.13, 0.13, 0.13, 0.13] * u.mas,
        "w_parallax": [0.0125, 0.0125, 0.0125, 0.0125] * u.mas,
        "pmphi1": [-6, -8, -8.5, -8.5] * u.mas / u.yr,
        "w_pmphi1": [1.5, 1.5, 1.5, 1.5] * u.mas / u.yr,
    }
)
prior_names = ("phi1", "phi2", "parallax", "pmphi1")
...
stream_astrometric_prior = sml.prior.ControlRegions(  # type: ignore[type-var]
    center=Data.from_format(  # type: ignore[type-var]
        ...
        fmt="astropy.table",
        names=prior_names,
        renamer={k: k for k in prior_names},
    ).astype(xp.Tensor, dtype=xp.float32),
    width=Data.from_format(  # type: ignore[type-var]
        stream_cp,
        fmt="astropy.table",
        names=tuple(f"w_{k}" for k in prior_names[1:]),
        renamer={f"w_{k}": k for k in prior_names[1:]},
    ).astype(xp.Tensor, dtype=xp.float32),
    lamda=1_000,
)
...
stream_astrometric_model = sml.builtin.Normal(
    ...
    coord_err_names=tuple(f"{k}_err" for k in coord_astrometric_names),
