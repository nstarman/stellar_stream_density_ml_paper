...
# Astrometric Nodes
pgm.add_node(
    "stream_sigma_w,obs", r"$\Sigma_n^{(w)}$", 2, 2, observed=True, plot_params=w_color
)
pgm.add_node(
...
# Photometric Nodes
pgm.add_node(
    "stream_sigma_m,obs", r"$\Sigma_n^{(m)}$", 5, 2, observed=True, plot_params=m_color
)
pgm.add_node(
...
# Full Data Node
pgm.add_node("stream_x,obs", r"$x_n^{obs}$", 3, 1, observed=True)
pgm.add_edge("stream_w,obs", "stream_x,obs")
...
# Full Data Node
pgm.add_node("x,obs", r"$x_n^{obs}$", base_shift + 3, 1, observed=True)
