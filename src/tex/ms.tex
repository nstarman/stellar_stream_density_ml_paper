...
\section{Method} \label{sec:method}
    ...
    \subsection{Photometric Model} \label{sub:photometric_model}
        ...
        \subsubsection{On-Stream} \label{sub:photometric_model_on_stream}
            ...
            % TODO? work in something to the effect of:
            % The PDMF doesn't have an enormous impact, regardless as the isochrone model is very thin and stars close to the isochrone photometrically are generally quite far astrometrically. This is an advantange of simultaneous modeling, where errors in one model are balanced by other parts of the model.
    ...
    \subsection{Priors} \label{sub:priors}
        ...
        \subsubsection{Region Prior} \label{sub:track_region_prior}
            ...
            % A good balance between these two goals is to have a prior that only influences the track on large scales.
...
\section{Results: Mock} \label{sec:results_mock}
    ...
    % We then apply the method to real stellar streams in \autoref{sec:results_gd1} and \autoref{sec:results_pal5}.
    ...
    % sub: test_on_a_synthetic_stream (end)
