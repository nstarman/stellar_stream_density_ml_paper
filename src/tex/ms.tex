    ...
    \subsection{Trained Density Model} \label{sub:results_mock:results}
        ...
        \begin{figure*}
            ...
            \caption{
                ...
                %
                \textbf{Lower panel 3: }%
                    $\parallax_2(\phi_1)$ over the full range of $\phi_1$. Like for
...
\section{Results: GD-1} \label{sec:results_gd1}
    ...
    we now apply the method to the stellar stream \stream{GD-1}.
    ...
    \subsection{Data Selection} \label{sub:results_gd1:data}
        ...
        % TODO: change the actual isochrone used in the code to -1.2, not -1.3
    ...
    \subsection{Model Specification}\label{sub:results_gd1:model}
        ...
        % TODO: talk about priors on the ishochrone, e.g PDM
        ...
        \begin{table}
            ...
            \begin{tabular}{@{}r<{\hspace{7pt}}*{4}{r<{\hspace{7pt}}}l<{\hspace{7pt}}@{}}
                ...
                \midrule
                stream & -70 & & & $_{14.2}^{14.8}$ & $_{0.11}^{0.14}$ \\ 
                stream & -60 & $_{-2.55}^{\phantom{+}0.95}$ & $_{-13.25}^{-11.75}$ & & \\ 
                stream & -50 & $_{-1.75}^{\phantom{+}1.75}$ & $_{-14.05}^{-12.55}$ & & \\ 
                stream & -40 & $_{-1.75}^{\phantom{+}1.75}$ & $_{-14.05}^{-12.55}$ & $_{14.1}^{ 14.7}$ & $_{0.11}^{0.15}$ \\
                spur & -35 & $_{0.45}^{2.15}$ & $_{-15.95}^{\phantom{0}-9.95}$ & & \\
                stream & -30 & $_{-1.75}^{\phantom{+}1.75}$ & $_{-13.35}^{-11.85}$ & $_{14.1}^{ 14.7}$ & $_{0.11}^{0.15}$ \\
                spur & -30 & $_{0.45}^{2.15}$ & $_{-15.60}^{\phantom{0}-9.60}$ & & \\
                spur & -20 & $_{0.65}^{2.35}$ & $_{-16.50}^{\phantom{0}-6.50}$ & & \\
                stream & 0 &  & & $_{14.7}^{15.3}$ & $_{0.09}^{0.15}$ \\
...
\section{Palomar 5} \label{sec:results_pal5}
    ...
    % TODO: some reiteration on the importance of Pal5
    ...
    \subsection{Data Selection} \label{sub:results_pal5:data}
        ...
        \todo[inline]{rephrase (numbers are correct, phrasing is from GD-1}

        We utilize data from \Gaia{} DR3 for the astrometric coordinates of each
        star.  To obtain more accurate color information, we cross-match the
        \Gaia{} field on \PanStarrs{} \citep{PanSTARRS1} using \Gaia's provided
        \texttt{Best-neighbors} catalog.  To limit the field to a region known
        to contain \stream{Pal\;5} we adopt broad cuts on the cross-matched data.
        We perform these rough cuts by requiring the data lie within the
        phase-space cube:
        \begin{itemize}
        ...
        For stars with missing parallax measurements we assume a distance of $8.5 \ \unit{kpc}$.  We include the $1-\sigma$ errors
        as a source of uncertainty in $\mbs{\Sigma}_n$, the per-star photometric
        uncertainty.
        We also correct the parallax for the zero-point using the \code{gaiadr3-zeropoint} package, which implements the findings from \cite{Lindegren+2021}.
        The result is a field populated by
        \variable{output/pal5/ndata_variable.txt} stars, from which we would like
        to identify stream members.

        We create data masks for a more refined selection around \stream{Pal\;5}.
        The two mask types, proper motion and photometric, are show in
        \autoref{fig:gd1-data_selection}.  The proper motion masks select
        rectangular regions, containing the primary overdensity of \stream{Pal\;5}
        % and a surrounding area following the tail of the distribution.  The
        % tighter mask (red) has corners $(\mu_{\phi_1}, \mu_{\phi_2}) =
        % [(-15,-10) (-4.5, -2)] \ \unit{mas/yr}$, while the larger mask (yellow)
        % has $(\mu_{\phi_1}, \mu_{\phi_2}) = [(-15,-5) (-5, -1)] \
        ...
        % The selection region is a 0.2 dex orthogonal buffer around the MIST
        % isochrone \citep[using][]{brutus}, which is illustrated in the
        % 2nd-from-the-top-left plot in \autoref{fig:gd1-data_selection}.
        In
        addition, to avoid detailed modeling of the completeness as a term in
        $\prior_{obs}$ (from \autoref{eq:isochrone_prior_components}) we mask out
        photometry with $G_{\Gaia{}} > 20$.  With combinations of these masks
...
\section{Probability Distributions} \label{app:distributions}
    ...
    \vspace{10pt}
    \subsection{Skew-Normal Distribution} \label{sub:skewnormal_distribution}
