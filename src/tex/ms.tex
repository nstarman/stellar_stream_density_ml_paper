...
\section{Method} \label{sec:method}
    ...
    \subsection{Likelihood Setup}\label{sub:likelihood_setup}
        ...
        \IfFileExists{static/DAG_StreamDensity.pdf}{
            \begin{figure}
                \centering
    ...
    \subsection{Photometric Model} \label{sub:photometric_model}
        ...
        \subsubsection{On-Stream} \label{sub:photometric_model_on_stream}
            ...
            % link to the code!
            In the code we offer a few ansatz for the PDMF. The default assumption, which is known to be non-physical is a uniform distribution over $\gamma$. Any user-defined function may be used
        ...
        \subsubsection{Linking to Astrometrics: the Distance Modulus and the Parallax}
            ...
            \begin{align}
                ...
                % \prior(\{\sigma_I, \sigma_\varpi \}(\phi_1)) &= \delta( \rm{dist}(\mu_I) - \rm{dist}(\varpi) )(\phi_1) \label{eq:distance_width_prior}
            \end{align}
            ...
            % TODO: include the specific functions?
            % \begin{align}
            %     \mu_I(\varpi) &= 10 - 5\log_{10}(\varpi) \\
            %     \sigma_I(\sigma_\varpi) &= \TODO{\text{write down the equation}}
            % \end{align}
