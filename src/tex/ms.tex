...
\section{Introduction} \label{sec:intro}
    ...
    \begin{quotation}
        ...
        galaxies or star clusters \citep{Odenkirchen2001, Majewski2003}. In the
        ...
        halo \citep{Lynden-Bell1967, Searle1978}; or partially, such as tidal
        ...
        points for a spherical galactic field \citep{Ross1997, Fukushige2000,
        ...
        cold streams, with smaller dispersions \citep{Johnston1998, Hendel2015}.
        ...
        large distances) \citep{Gibbons2014}. However, an orbit is impractical
        ...
        known streams \citep[e.g. for \stream{Palomar 5} alone:][]{Rockosi2002,
        Ibata2017, Grillmair2006, Carlberg2012, StarkmanEtAl2019}, and increase
    ...
    \paragraph{similar orbits and a somewhat noisy population}
        ...
        \cite{2018MNRAS.477.4063M}, with a distribution radial to the track.
...
\section{Method} \label{sec:method}
    ...
    \subsection{Framing the Data} \label{sub:method:framing_the_data}
        ...
        8$ kpc distant \citep{GRAVITY2018, Leung2022} in a local frame,
        ...
        around the Milky Way \citep{Wannier1972}, no such transformation is
        ...
        pre-processing \citep[see][\S2.1]{StarkmanEtAl2023}.  We note that, with
    ...
    \subsection{Likelihood Setup}\label{sub:method:likelihood_setup}
        ...
        \citep{McLachlan1989MixtureM}.  Mixture Density Networks (MDNs) are a
    ...
    \subsection{Photometric Model} \label{sub:method:photometric_model}
        ...
        \subsubsection{On-Stream} \label{sub:method:photometric_model:on_stream}
            ...
            \citep{2001MNRAS.322..231K}. The Kroupa IMF, and all
    ...
    \subsection{Priors} \label{sub:methods:priors}
        ...
        setup. In \autoref{sub:method:photometric_model:on_stream} we added
    ...
    \subsection{Model Implementation, aka the code} \label{sub:methods:model_implementation}
        ...
        \!\!\textrightarrow \texttt{Dropout} blocks. This is illustrated in
...
\section{Results: Mock} \label{sec:results_mock}
    ...
    In this section we apply our model to a synethic stream in order to
    demonstrate an accurate recovery of stream members and density variations.

    We generate synthetic observations by sampling points from a third degree
    polynomial \JN{check this} with a constant scatter. Density variations are
    then simulated by evaluating the stream density with two Gaussian
    ...
    density. The background astrometric  points in are generated from
    exponential distributions at each $\phi_1$. The number of stream stars is
    $\variable{output/mock/nstream_variable.txt}$, while the number of
    background stars is $\variable{output/mock/nbackground_variable.txt}$.
    ...
    The synthetic stream is also simulated in magnitudes using a 12~Gyr
    isochrone with [Fe/H] = -1.35~dex. The isochrone truncates just short of the
    giant branch, and sampled from uniformly (i.e., the initial mass function,
    $P(\gamma)$, is uniform). The background photometric coordinates are
    generated as a 2-dimensional Gaussian with non-zero covariance. The distance
    modulus is derived from a polynomial parallax track with uniform thickness.
    Thus, the synthetic data scenario described here generates a mock stream in
    ...
    The are additional variations  mostly around the edges of the stream, though
    ...
    photometric coordinates, with $x\%$ stream stars being recovered with
    membership probability greater than $y$. The false positive rate (i.e.,
    stars misidentified as stream members) for this test is found to be $z\%$
    when cutting on this threshold. 
...
% sec: test_on_a_synthetic_stream (end)

\section{Results: GD-1} \label{sec:results_gd1}

    We now apply our density modeling approach to the stellar stream GD-1. We
    discuss the data selection in \autoref{sub:results_gd1_data} and results in
    \autoref{ssub:trained_gd1}.
    
    \subsection{Data Selection} \label{sub:results_gd1_data}
