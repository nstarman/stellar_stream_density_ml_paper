...
\section{Method} \label{sec:method}
    ...
    Even non-astrometric coordinates, e.g. photometry and metallicity, are modelled along the stream and background as functions of $\phi_1$.
    ...
    \subsection{Framing the Data} \label{sub:framing_the_data}
        ...
        Within a Galactocentric framework and expressed in spherical coordinates, streams
        effectively approximate linear formations on a great circle, wherein a phase angle
        traverses the entire stream length.

        
        % Data observations are taken in an (approximately) heliocentric reference frame, e.g. from the Earth or a Lagrange point, and is reported in an ICRS
        % \citep{ICRS1997} frame. Restricted as we are to a very local volume, there
        % is no means to observe systems from more ``intrinsic" reference frames, namely from the Galactic center of mass about which orbit satellites such as stellar streams.

        % As the progenitor orbits the Galactic COM (center of mass), its
        ...
        % (approximately) along a galactocentric ellipse there is no guaranteed rotation to similarly align the stream in the observer coordinates so that the stream
        % lies along $\phi_2(\phi_1) \approx 0$. Thankfully, many streams are sufficiently
        ...
        % In a Galactocentric frame and in spherical coordinates  streams are approximately lines on a great circle, with a phase-angle sweeping the length of the stream. This, clearly, is a good affine parameter along which to model and study the stream.
    ...
    \subsection{Likelihood Setup}\label{sub:likelihood_setup}
        ...
        \begin{figure}
            ...
            \todo[inline]{todo}
            \begin{enumerate}
                \item Do this with \url{https://github.com/daft-dev/daft}
            \end{enumerate}
            \label{fig:DAG}
        \end{figure}
    ...
    \subsection{Photometric Model} \label{sub:photometric_model}
        ...
        \subsubsection{On-Stream} \label{sub:photometric_model_on_stream}
            ...
            % link to the code!
            In the code we offer a few ansatz for the PDMF. The default assumption, which is known to be non-physical is a uniform distribution over $\gamma$.
            Any user-defined function may be used.
        ...
        \subsubsection{Linking to Astrometrics: the Distance Modulus and the Parallax}
            ...
            \begin{align}
                ...
                \sigma_I(\sigma_\parallax) &= \frac{5}{\ln{10}} \mu_I \sigma_\parallax
            \end{align}

            \todo[inline]{expand on this?}

            In practice, at the distance of many streams the errors in the parallaxes are of order 1 and we excluded the feature from most models.
            % TODO! 
    ...
    % \subsection[Astrometric Background]{Astrometric Background: pieces of $\Bcmp{P}(\mbs{w})$} \label{sub:background_astrometric}
