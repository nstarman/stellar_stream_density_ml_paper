...
\section{Results: Mock} \label{sec:results_mock}
    ...
    density. The background astrometric points in are generated from exponential
    distributions at each $\phi_1$.  The number of stream stars is
    ...
    The synthetic stream's photometry is also simulated using a 12~Gyr, [Fe/H] =
    -1.35~dex NMIST isochrone \TODO{[cite brutus]}. The isochrone truncates just
    ...
    We pre-train a normalizing flow (see
    \autoref{sub:method:photometric_model:off_stream}) on an off-stream
    selection.  We train on the photometry.  You can see the selection in
    \autoref{fig:mock_data_photometric_background_selection}.  You can see both
    the on-stream and off-stream labels, blue and black respectively.  We select
    using an astrometric selection. In the photometry you can see that the
    on-stream selection has two populations, drawn from the isochrone and drawn
    from the background The off-stream selection, meanwhile, has only the
    background population. Next we train the full model.
